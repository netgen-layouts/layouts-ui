define(['underscore', './main', 'app'], function(_, Blocks, App){
  ...
  return {
    init_group_block: function(data){
      ...
      var block_type = App.g.block_types.findWhere({kind: data.kind});
      var type = (block_type && block_type.id) || 1;
      var attributes = _.defaults({identifier: type, in_group: true, label: data.label}, data.attributes);
    ...
    init_block_from_type: function(type, additional_attributes){
      ...
      var attributes = _.defaults({definition_identifier: type.get('defaults').definition_identifier}, type.get('parameters'), additional_attributes);
      return new Klass(attributes);
    },

    init_block: function(params){
      var block_type = App.g.block_types.get(params.block_type);
